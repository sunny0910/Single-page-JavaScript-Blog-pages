...
$('#pages-ul').on('click','.left-arrow',function(){
	if(flag==0){//search_recent(arr);
		current_page--;
		if(current_page>3){search_start--;
							search_end--;}
...
if(flag==0){//search_recent(arr);
	current_page++;
	if(current_page<=search_pages-3){ search_start++;
	search_end++;}
	search_recent(arr);
	search_pagination(arr);
}
else {
	if(current_page==no_of_pages) return false;

	current_page++;
	if(current_page<=no_of_pages-3){ page_start++;
									page_end++;}
	create_recent(current_page);
	pagination();
...
if(flag==0){
		id=$(this).attr('id');
        if(id==current_page) return false;
		current_page=id;
...
$('#search-input').on('keyup',function(){
  ...
  //var current_page=1;
  if(text == '') { flag=1;$('#recent_h1, .recent, .pages').css('display','block'); $('.noresult').css('display','none'); create_recent(current_page); pagination(); return false}
  flag=0;
  search(posts,text);
...
function search_pagination(obj) {
	...
	search_pages= pages
	//search_start=1;search_end=5;
	var j;

	if(current_page>=3)$('#pages-ul').append("<button><li class='left-arrow'><a><i class='fa fa-chevron-circle-left' aria-hidden='true'></i></a></li></button>");
	for (j=search_start;j<search_end+1;j++)
	{  
		if(j==current_page){$('#pages-ul').append("<button><li><a class='active-button' id='"+j+"'>"+j+"</a></li></button>");continue;}
		$('#pages-ul').append("<button><li><a class='buttons' id='"+j+"'>"+j+"</a></li></button>");
	}
	if(current_page<=pages-2)$('#pages-ul').append("<button><li class='right-arrow'><a><i class='fa fa-chevron-circle-right' aria-hidden='true'></i></li></button>");
...
function pagination() {
	...
	if(current_page>no_of_pages-2) {page_end=no_of_pages;page_start=current_page-3;}
	if(current_page<3){page_start=1;page_end=5;}
	...
	if(current_page<=no_of_pages-1)$('#pages-ul').append("<button><li class='right-arrow'><a><i class='fa fa-chevron-circle-right' aria-hidden='true'></i></li></button>");
...
function featured_posts() {
	...
	{  
		if(count<4 && posts[i]['is_featured']==1)  
			{	if(count === 0) { $('.ls').append("<img class='pr' src='"+posts[i]['img']+"'>"); count++;}
				else 
					if(count=== 1 )  { $('.ls').append("<img src='"+posts[i]['img']+"'>"); count++;}
				else
					if(count=== 2)   { $('.rs').append("<img class='pr' src='"+posts[i]['img']+"'>"); count++;}
				else
					if(count=== 3 )  { $('.rs').append("<img src='"+posts[i]['img']+"'>"); count++;}
			}
		if(count==4) break;
	}
